...
export const saveCompanyProfile = async (req, res) => {
  try {
    ...
    const { company_name, email, phone, gst_number, cin_number, address, city, state, pincode, industry } = req.body;
    ...
    if (existing.rows.length > 0) {
      await pool.query(
        `UPDATE company_info
         ...
         WHERE user_id=$11`,
        [company_name, email, phone, gst_number, cin_number, address, city, state, pincode, industry, userId]
      );
    ...
    await pool.query(
      ...
      [userId, company_name, email, phone, gst_number, cin_number, address, city, state, pincode, industry]
    );
...
export const getCompanyProfile = async (req, res) => {
  try {
    ...
    res.json(result.rows[0] || { message: "No company profile found" });
  } catch (error) {
